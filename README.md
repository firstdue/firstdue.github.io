...
35 facades. v18y continues that corridor from the owner's screenshots: a level stone viaduct arcade,
concrete barriers and a median on Lincoln Drive, painted gore chevrons, photo-sourced gantries and
corridor street lighting. **PUBLIC = LOCAL, and the owner's authoring folder is the source of truth** — v18t–v18y were authored
...
markers in rows on the lawns near the truck — generic markers, no names. Along the electrified rail lines,
catenary masts stand every 55 metres with cantilever arms over the track and a wire strung between them, so the
SEPTA lines read as electric railways; third rail is left unmarked. Everything thins with distance to protect
the phone, and no performance budget moved. Buildings now also keep off cemetery grounds.
...
trees on the lawns, so Fairmount Park, Hunting Park and the neighbourhood squares read as what they are. No new
data; scenery only. Cemeteries are the next step, since the map data does not yet carry them.
...
side table that says, block face by block face, whether a street is lined with row homes, twins, detached
houses, apartments, shops or industry. Row-home streets now render as attached runs with a repeating facade and
cornice, twin streets as gabled pairs with porches, and detached streets as set-back houses with a gable. Shops,
apartments and industry keep the generic building for now. Scenery only: the road graph and driving are
unchanged, the world uses fewer triangles than before, and if the table ever fails to load the generic
buildings return. The raw parcel data stays in the authoring folder; only the 120 KB table ships.

**v19f (September 18, 2026) — graphics step 1: real guardrails and clean ramp edges.** The flat box rails along
the expressway, the Ridge-to-City viaduct and the City Avenue / Lincoln Drive corridor are now tiled W-beam
guardrail panels with posts and lap splices, all from one shared piece. The interchange ramps get a continuous
mitred edge strip and a smoothly graded embankment in place of the stepped per-segment edges. Scenery only: the
road graph, driving surfaces, heights and markings are unchanged. This is the first release measured against the
new performance gate; draw calls, memory and the rebuild hitch did not move.
...
Avenue / Green Lane exit. Both were the map bake clipping roads at the city limit: an interchange is one
structure and it straddles that line, so cutting it removed real exits. Freeway roads are now kept further out,
along with any street a ramp actually joins, and eastbound traffic gets exit 340A for Lincoln Drive and Kelly
Drive while westbound gets exit 338 for Belmont Avenue and Green Lane. Exit wording comes from the map's own
tags. Mileposts stop where the surveyed markers stop, as before.

**v19c (September 13, 2026) — hydrant button gated, guide fixed citywide.** Marking a hydrant is now limited to
accounts that have been granted it, because the button was easy to press by accident in play; the rest of the
feature is unchanged and can be handed back per account. The blue advisory guide no longer breaks into pieces
away from the City Avenue corridor — it is held above the finished pavement everywhere, and at a junction it
clears the crossing street's surface as well as its own.

**v19b (September 13, 2026) — interchange ramp detail.** The ramps at City Avenue / Lincoln Drive / Kelly
Drive carried no lane markings at all, which is why they read as bare grey slabs. A ramp is a one-way
carriageway, so it now has a yellow line on the left and a white line on the right, and where it runs on the
ground it gets a paved shoulder and a graded earth slope out to the terrain instead of ending abruptly in mid
air. Where the ramp is up on its deck the existing parapets still do that job. Scenery only — the road graph,
road heights and every marking on a named street are unchanged.
...
many junctions ahead it previews (two) rather than by how far it looks — so a card still appears well before
its junction, but four junctions' worth no longer pile up on a phone screen at once.

**v18y (September 13, 2026) — corridor pass 2.** From the same owner screenshots: the railway viaduct is
a level run of stone arches either side of the Lincoln Drive opening; Lincoln Drive between Ridge Avenue
and the viaduct has concrete barrier walls and a continuous median; white chevrons mark the gores where
ramps split or merge; Lincoln SB, Kelly NB and City Ave carry the photographed gantries ("Ridge Ave NORTH
EXIT ONLY", "Lincoln Drive NO TRUCKS", "I-76 WEST Valley Forge NEXT LEFT / Lincoln Drive"); the corridor
roads and ramps get street lighting. Sign panels the road graph cannot honour were left out. Scenery and
signs only — graph, heights and routing unchanged.
